<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>

<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>

<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Contact</title>
<!-- Lier lb bootstrap à ma page -->
<link rel="stylesheet" href="../assets/css/bootstrap.css" />
<link rel="stylesheet" href="../assets/css/BoVoyageStyle.css" />
</head>
<body>
	<!--  inclure le header -->
	<%@include file="/templates/headerPublic.html"%>

	<h1 id="titre">Veulliez remplir ce formulaire afin de nous
		contacter</h1>
	<br />
	<br />
	<div id="login">
		<div class="row">
			<form:form class="form-horizontal " method="POST"
				modelAttribute="contact" action="sendSimpleEmail">
				<label class="col-md-3" for="name">Nom :</label>
				<form:input class="col-md-5" id="name" path="name" />
				<br />
				<br />
				<br />
				<br />
				<label class="col-md-3" for="email">E-mail :</label>
				<form:input class="col-md-5" id="email" path="email" />
				<br />
				<br />
				<br />
				<br />
				<label class="col-md-3" for="subject">Objet :</label>
				<form:input class="col-md-5" id="subject" path="subject" />
				<br />
				<br />
				<br />
				<br />
				<label class="col-md-3" for="message">Votre message :</label>
				<form:textarea class=" col-md-5" id="message" rows="10" path="text"></form:textarea>
				<br />
				<br />
				<br />
				<br />
				<br />
				<br />

				<div class="col-md-7">
					<br /> <input class="btn btn-success" type="submit"
						value="Envoyer">
				</div>

			</form:form>
		</div>
	</div>
	<!--  inclure le footer -->
	<%@include file="/templates/footer.html"%>
</body>
</html>